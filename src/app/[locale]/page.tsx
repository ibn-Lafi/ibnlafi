...
export default async function LocalePage({ params }: PageProps<"/[locale]">) {
  ...
  const { hero, contact, nav, experience, education } = getContent(locale);
  const ar = locale === "ar";
  const copy = ar ? {
    skip: "انتقل إلى المحتوى", work: "أعمال ومشاريع", experience: "الخبرات", contact: "لنتواصل",
    ...
    experienceNote: "تجارب متنوعة، يجمعها التطوير والعمل على أرض الواقع.",
    skills: ["تطوير الأعمال", "تطوير المنتجات", "بناء العلامات التجارية", "الذكاء الاصطناعي والأتمتة"],
    contactTitle: "عندك فكرة؟ لنتحدث.", note: "يسعدني التواصل حول المشاريع وفرص العمل.", location: "مكة المكرمة، السعودية", details: "التفاصيل",
  } : {
    skip: "Skip to content", work: "Selected work", experience: "Experience", contact: "Let’s connect",
    ...
    experienceNote: "Different industries. A practical focus on building and improving.",
    skills: ["Business development", "Product development", "Brand building", "AI & automation"],
  ...
  return (
    <div className="portfolio">
      ...
      <header className="site-header">
        ...
        <nav className="section-nav" aria-label={ar ? "أقسام الصفحة" : "Page sections"}><a href="#work">{copy.work}</a><a href="#experience">{copy.experience}</a></nav>
      ...
      <main id="main">
        ...
        <section id="experience" className="page-section" aria-labelledby="experience-title">
          <div className="section-heading"><div><p className="section-label">02 / {copy.experience}</p><h2 id="experience-title">{copy.experience}</h2></div><p>{copy.experienceNote}</p></div>
          <div className="experience-layout"><div className="experience-list">{experience.items.map((item,index)=><details className="experience-item" key={item.organization}><summary><span className="experience-mark" aria-hidden="true">{index===0 ? <Image src="/brands/dulceur.png" alt="" width={64} height={28} /> : <span>0{index+1}</span>}</span><span className="experience-summary"><strong>{item.role}</strong><span>{item.organization}</span><small>{item.period}</small></span><span className="expand-icon" aria-hidden="true">+</span></summary><ul>{item.description.map(x=><li key={x}>{x}</li>)}</ul></details>)}</div><aside className="skills-panel"><h3>{ar ? "مجالات عملي" : "My focus"}</h3><ul>{copy.skills.map(x=><li key={x}>{x}</li>)}</ul><div className="education-note"><p>{education.items[0].degree}</p><span>{education.items[0].institution}</span></div></aside></div>
        </section>
        <section id="contact" className="contact" aria-labelledby="contact-title"><div><p className="section-label">03 / {copy.contact}</p><h2 id="contact-title">{copy.contactTitle}</h2><p>{copy.note}</p></div><div className="contact-links"><a className="contact-button primary" href={`mailto:${contact.email}`}><MailIcon className="h-4 w-4" />{contact.emailLabel}</a>{contact.whatsapp && <a className="contact-button" href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noopener noreferrer"><WhatsAppIcon className="h-4 w-4" />{contact.whatsappLabel}</a>}</div></section>
      </main>
